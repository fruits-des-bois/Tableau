...
<thead><tr><th>Date (UTC)</th><th>Débit (m³/s)</th><th>Continuité</th></tr></thead>
<tbody></tbody>
</table>
<script>
...
fetch(url)
  ...
  .then(j=>{
    ...
    j.data.forEach(it=>{
      ...
      tr.innerHTML=`<td style="text-align:left">${d}</td><td>${it.resultat_obs ?? "—"}</td><td style="text-align:center">${it.continuite_obs_hydro ?? "—"}</td>`;
